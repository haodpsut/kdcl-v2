...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=south, opacity=0.05] at (current page.south){\includegraphics[width=0.7\paperwidth]{dau-campus.png}};
  \node[anchor=north west] at ([xshift=0.55cm, yshift=-0.45cm]current page.north west){\includegraphics[height=0.9cm]{dau-logo.png}};
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=south east, opacity=0.10] at ([xshift=-0.2cm,yshift=0.1cm]current page.south east){\includegraphics[width=0.55\paperwidth]{dau-campus.png}};
  ...
  \node[anchor=north west] at ([xshift=0.6cm, yshift=-0.55cm]current page.north west){\includegraphics[height=1.0cm]{dau-logo.png}};
...
\vspace{0.12cm}
\begin{keypoint}
Hệ thống hỗ trợ song song \textbf{kiểm định cơ sở giáo dục} (TT20, 15 tiêu chuẩn) và \textbf{kiểm định chương trình đào tạo} (TT04, chuẩn đầu ra), trên cùng một nền tảng quản lý minh chứng.
\end{keypoint}
\end{frame}
...
\scriptsize
\begin{itemize}
  \item Theo Thông tư 20 (bộ tiêu chuẩn hiện hành)
...
\scriptsize
\begin{itemize}
  \item \textbf{Thông tư 20/2026/TT-BGDĐT} — bộ tiêu chuẩn đánh giá chất lượng cơ sở giáo dục (15 tiêu chuẩn / 60 tiêu chí)
...
\midrule
Cơ sở pháp lý & Thông tư 20 & Thông tư 04 (AUN-QA) \\
...
\begin{tikzpicture}[remember picture, overlay]
  \fill[daunavy] (current page.north west) rectangle (current page.south east);
  \node[anchor=south east, opacity=0.10] at ([xshift=-0.2cm,yshift=0.1cm]current page.south east){\includegraphics[width=0.5\paperwidth]{dau-campus.png}};
  \node[anchor=north west] at ([xshift=0.6cm, yshift=-0.6cm]current page.north west){\includegraphics[height=1.0cm]{dau-logo.png}};
